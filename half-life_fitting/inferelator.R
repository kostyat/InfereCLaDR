...
if (length(args) > 1) {
  ...
  PARS$save.to.dir <- paste(save.to.dir, paste("prior_seed_",PARS$job.seed.prior,sep=''), paste("clust",clustn,sep=''), std.last.new, sep='/')
}
...
#This is only for the parameter-fitting mode, when we don't need to re-save the same input again for every single tau.
if(PARS$tau == 0) {
  save(PARS, IN, SEED, PROCTIME, file = paste(PARS$save.to.dir, "/params_and_input.RData", sep=""))
} else {
  save(PARS, SEED, PROCTIME, file = paste(PARS$save.to.dir, "/params_only.RData", sep=""))
}
