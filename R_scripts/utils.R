...
##### InfereCLaDR note: load.expr.meta.degrates.from.output needs to be set to TRUE to load expression, meta data, and degradation rate files from the output of the clustering and degradation rates fitting steps. Otherwise, they will be loaded from input.dir
read.input <- function(input.dir, exp.mat.file, tf.names.file, meta.data.file, 
           ...
           load.expr.meta.degrates.from.output) {
  IN <- list()
  
  if(load.expr.meta.degrates.from.output) {
    exp.mat.file.path = file.path(exp.mat.file)
    meta.data.file.path = file.path(meta.data.file)
    deg.rates.file.path = file.path(deg.rates.file)
  } else {
    exp.mat.file.path = file.path(input.dir, exp.mat.file)
    meta.data.file.path = file.path(input.dir, meta.data.file)
    deg.rates.file.path = file.path(input.dir, deg.rates.file)
  }
  
  if (grepl('.RData$', exp.mat.file)) {
    IN$exp.mat <- local(get(load(exp.mat.file.path)))
  } else {
    IN$exp.mat <- as.matrix(read.table(file=exp.mat.file.path,
  ...
  if (!is.null(meta.data.file)) {
    if (grepl('.RData$', meta.data.file)) {
      IN$meta.data <- local(get(load(meta.data.file.path)))
    } else {
      IN$meta.data <- read.table(file=meta.data.file.path, header=T, sep='\t', check.names=F)
    }
  }
  ...
  if(!is.null(deg.rates.file)) {
    IN$deg.mat <- NULL
    IN$deg.mat <-  as.matrix(read.table(file=deg.rates.file.path,
                                    row.names=1, header=T, sep='\t', check.names=F))
  }
